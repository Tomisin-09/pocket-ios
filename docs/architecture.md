...
- **`Profile`** (ADR 0113 Slice 1) is the local, account-free artist profile — a **singleton** `@Model`
  ...
  **completed an exercise or captured a loop** (a Home-appearance check gated by the
  `artistNamePromptSeen` UserDefaults flag). Nothing leaves the device; it is deliberately not treated as PII. Slice 2's
  planner-curation fields land later as additive optionals, with any enums backed by primitives (the
