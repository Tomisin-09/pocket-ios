...
- **My Progressions** (`MyProgressionsView`, ADR 0218) is the same split for written progressions:
  a newest-first list of `SavedProgression`s, each row pushing `ProgressionBuilderView` to edit or
  delete, **+** to write one. The *Use a progression* sheet lists the same `@Query` to insert from.
...
- **`SavedProgression`** (ADR 0218 D10) is `SavedChord`'s pattern applied to a progression the player
  writes: `uid`, `name`, `createdAt` and `stepsData` — a `SavedProgressionPayload` blob (`version`,
  `[ProgressionStep]`, the `tonic` it was written in) decoded tolerantly, so a missing key or a quality
  this build doesn't know degrades rather than failing. A new entity with declaration-defaulted
  attributes, so registering it is additive (ADR 0189's D1–D3 don't engage). No relationships: a drill
  filled from one holds **copies** of its chords, so editing or deleting the progression never touches
  a drill. Written in `ProgressionBuilderView` over the pure `ProgressionDraft` (whose `setTonic`
  re-expresses steps from the new tonic — the key is corrected, never transposed), inserted from the
  *Use a progression* sheet's **Your progressions**, and managed in the Toolkit's `MyProgressionsView`.
...
- **A shared routine is the second file-out path** (ADR 0188 S1, `SharedPractice*.swift`). A routine
  ...
  - **And one optional progressions field** (ADR 0218 D10): `PracticeArchive.savedProgressions`
    (`[SavedProgressionRecord]?` — `uid`, `name`, `createdAt`, and the payload as a nested
    `JSONValue`, exactly as a saved chord's voicing travels). Restore follows the saved chords' rules:
    a uid already present is left alone, a row whose payload won't re-encode is skipped rather than
    landed empty, and — unlike folder markers and custom skills — a saved progression **is** a library
    item, so it counts in `rowCount` and has its own *Saved progressions* line in `RestorePlan`. It
    does not cross the share doors, as saved chords don't.
