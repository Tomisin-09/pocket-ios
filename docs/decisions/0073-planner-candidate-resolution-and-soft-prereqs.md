...
- **Amended by:** ADR 0216 (2026-09-11). **Decision 4** — a drill's type becomes a **default** it
  can narrow or expand, per drill (0216 D1, built in its slice 2; until then resolution is still
  type-only). **§4 / Decision 7** — a goal may also name a **custom skill** the player creates (0216
  D7, slice 2). Path A / Path B and the soft prerequisite stage stand as written.
