...
- **Amended by:** ADR 0209 — the `.redmoonpractice` file gains its **second payload kind**, an
  ...
  **One decision here is reversed:** the Pro gate below runs *before* the file is read, and 0209 D5
  moves it *after* — an exercise's gate is asked per **template**, which is a fact inside the file,
  so the file must be read to know which question to ask. (Every template is Pro today, so the
  verdict is unchanged on every file that exists; the template-shaped question is kept for the
  free-tier seam.) A valid file walls exactly as it did; a corrupt or future-version file now
  reports itself rather than presenting a paywall for a file that was never going to open.
  **Everything else stands:** D1's trust asymmetry, D2's version gate (the number is deliberately
  *not* bumped for a new kind), D3's file type and its `Info.plist` declaration, D4's placeholders,
