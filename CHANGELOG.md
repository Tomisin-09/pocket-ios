...
### Held back — built, not shipped

> **Do not move these into a released version's section.** The Red Moon Oracle is finished as a
> mechanism and unfinished as a voice: the reading it draws passes every guard and still reads like
> an engineer describing a data structure, so **ADR 0211** closed its door on Home rather than ship
> prose that was rejected on sight. The code, the guards and their nine test files all stay in the
> tree; nothing in the app reaches the screen. These two entries stay here until the register lands
> — release notes describing a screen nobody can open are the same mistake as a *Coming soon* tile,
> made in prose. `docs/backlog.md` holds what reopens it.
...
- **A song to start on, before you decide anything.** Red Moon now arrives with one real track —
  *Binta* by Jack Trader, eighty-one seconds, written and recorded by us — and a **Start here** card
  ...
  its bar lines drawn and two markers — *Chords start* and *Solo start* — either side of the four
  bars most worth looping first, so the metronome works from the first note. You can loop it, slow
  it right down and save that loop **without Red Moon Pro**, because working out whether an app
...
- **Your first song shows you what the practice screen is for.** The first song you bring in — or
  *Binta*, from the Start here card — opens with a small card under the transport and three things
  ...
  begins, so your two Loop taps land right on the bar lines. Once those four bars are going round,
  a ring on the metronome says you can hear the beat under them. After you save the loop, a ring on
  its row says those four bars make a good backing track, and Edit loop opens at that switch. Both
  are pointers, not steps: take them or close them, they don't come back, and nothing waits on them.
  Saving that first loop is marked once, and never again. If you told the opening questions you're comfortable or have been playing a
...
### Changed
- **Home is a screen again, not a menu.** The five destinations — **Practice**, **Metronome**,
  **Song library**, **Journal** and **Toolkit** — are now tiles, two to a row, still under the
  headings they were already grouped by. Each keeps its own colour, which is how you
  ...
  back is height: they take three rows where they took five, and that is what paid for
