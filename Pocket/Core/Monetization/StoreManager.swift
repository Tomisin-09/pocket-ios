...
final class StoreManager {
    ...
    /// wins when present, otherwise Pro follows a real entitlement and nothing else. `nonisolated` —
    /// it touches no actor state, so gates/tests can call it from any context.
    ///
    /// A third input lived here through the 2026-08 closed beta: a `betaGrant` flag that entitled any
    /// sandbox/TestFlight install outright. It is gone with the rest of that grant — the rule this
    /// ships with is "entitled, or a Debug override", and there is no path that unlocks Pro without
    /// StoreKit saying so. See `docs/plans/beta-testing-plan.md` for what the grant cost.
