...
#Preview("Waveform — Navigate") {
    ...
    return ZStack {
        ...
        WaveformView(amplitudes: Song.sample().amplitudes,
                     ...
                     loop: loops.first,
                     loops: loops,
                     markers: [WaveformMarker(fraction: 0.27, label: "Intro"),
                               WaveformMarker(fraction: 0.6, label: "Chorus")],
...
#Preview("Waveform — zoomed + Fit reset") {
    ...
    ZStack {
        ...
        WaveformView(amplitudes: Song.sample().amplitudes,
                     ...
                     loops: Song.sample().loops,
                     markers: [WaveformMarker(fraction: 0.42, label: "Solo")],
                     formingStart: nil,
                     tapSelection: nil,
                     playheadLabel: "0:24",
                     onSeek: { _ in },
    ...
    ZStack {
        ...
        VStack(spacing: 16) {
            LoopsPanel(loops: song.loops, expanded: $loopsExpanded,
                       ...
                       onAdjustRange: { _ in }, onAutomator: { _ in })
...
#Preview("Loops — empty") {
    ...
    ZStack {
        ...
        LoopsPanel(loops: [], expanded: $expanded, activeLoopID: nil, isPlaying: false,
                   onActivate: { _ in }, onEdit: { _ in }, onDelete: { _ in },
                   onAdjustRange: { _ in }, onAutomator: { _ in }).padding()
    }
}
        ...
        VStack(spacing: 16) {
            ...
            WaveformView(amplitudes: song.amplitudes,
                         ...
                         markers: song.markers.map {
                            WaveformMarker(fraction: $0.seconds / song.duration, label: $0.label) },
                         formingStart: nil,
                         tapSelection: nil,
                         playheadLabel: "0:10",
                         onSeek: { _ in },
