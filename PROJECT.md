...
| `Pocket/Core/Monetization/` | **Red Moon Pro entitlement** (ADR 0112). Pure `AccessPolicy` (`canAuthor` / `canRun` + `ExerciseTemplate.authoringTier`: free = basic/strumming/warm-up, Pro = the structured catalog) — the one place the free/Pro line lives, `isPro` passed in, Foundation-only + exhaustively unit-tested; plus `freeTasteSlugs` / `isFreeTaste` (the four run-free-forever seeded presets keyed by `Exercise.presetSlug`). **`StoreManager`** (`@MainActor @Observable`, StoreKit 2) is the single entitlement source of truth — resolves `isPro` from `Transaction.currentEntitlements` + a `Transaction.updates` listener, `loadProducts`/`purchase`/`restore`, injected at the app root via `.environment`; the pure `resolveIsPro(entitled:debugOverride:)` keeps the decision testable and honours a DEBUG override. Local `Configuration/RedMoonPro.storekit` (Annual £39.99 / Monthly £4.99 + 14-day intro, group "Red Moon Pro") is wired into the run scheme for ASC-free testing. Gate at **read** time — nothing about Pro persisted on a model |
