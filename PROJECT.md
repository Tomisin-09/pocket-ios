...
| `Pocket/Features/Practice/` | Top-level Practice hub → two unit libraries (`ExerciseLibraryView`, `LoopLibraryView`); template-first exercise **creation** (`ExerciseTemplatePicker` → `NewExerciseSheet` configure step, ADR 0068); per-unit training-run screens (`ExerciseRunView` / `LoopRunView` + `LoopRunModel`), an exercise **detail/reference sheet** (`ExerciseDetailSheet` — read-only **template** + editable description + read-only tempo/meter/routine, and a "How to play" section hosting the per-template content editor — `StrumPatternEditor` for strumming, `FretboardRunEditor` (declare a movable finger-pattern run + live preview) for the warm-up families and `FretboardDrillEditor` (tap-to-place notes) for the Scales custom fallback, ADR 0065; ⓘ from the run screen); the content-template renderers (`StrumLane` + live `StrummingLaneView`; `FretboardGrid` + live `FretboardView` + self-driving `FretboardDrillPreview`, ADR 0065 build 2); the library grouped into **template sections** (`PracticeLibrarySort.exerciseSections`, ADR 0068); a collapsible **Practice Settings** panel (`PracticeSettingsPanel`) grouping the run-setup tempos + Steps + curated starter exercises seeded once on first launch in three one-time batches — six v1 technique drills + a v2 strumming preset + a v3 fretboard preset (`PracticePresets`, ADR 0046/0065) |
...
| `Pocket/Core/Models/` | Song, Loop, Marker, JournalEntry, Exercise (+ closed `ExerciseTemplate` axis chosen at creation & immutable, deriving the `ExerciseKind` renderer + `StrumPattern` / `FretboardContent` (`FretboardRun` recipe | custom `FretboardDrill`) payloads, ADR 0068/0065), Routine, Session, SongRef |
...
timing math; `.fretboard` → `FretboardView`, notes walking a string × fret grid driven by the pure
`FretboardDrill` timing math — the shared surface for Scales/Picking/Legato/Fingerstyle/Warm-up,
ADR 0065 build 2), with an absent/undecodable payload falling back to the dots. The `Exercise` model stores its `CommandRamp`
